...
<div class="card card-primary">
    ...
    <form role="form" action="{{route('personnel.store')}}" method="POST" enctype="multipart/form-data">
        {{ csrf_field() }}

      <div class="card-body">
        <div class="form-group">
          <label for="title">نام و نام خانوادگی</label>
          <input type="text" name="name" class="form-control" id="exampleInputEmail1" placeholder="نام را وارد کنید" value="{{ old('name')}}">
        </div>
        <div class="form-group">
            <label for="title">سمت</label>
            <input type="text" name="post" class="form-control" placeholder="سمت را وارد کنید" value="{{ old('post')}}">
          </div>
